...
import logging
import feedparser
import calendar # Ajouté pour corriger le bug de date
...
def is_fresh(entry) -> bool:
    ...
    try:
        ...
        # ✅ Correction ici : Utilisation de calendar.timegm au lieu de mktime_tz
...
def can_push() -> bool:
    if not os.path.exists(THROTTLE_FILE): return True
    try:
    ...
    try:
        ...
        if feed.entries:
            for entry in feed.entries:
                if len(fresh_urls) >= MAX_URLS_PER_RUN: break
                if is_fresh(entry):
                    url = entry.link.split('?')[0].split('#')[0].rstrip('/')
                    if url not in fresh_urls: fresh_urls.append(url)
